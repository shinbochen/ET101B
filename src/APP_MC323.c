...
unsigned char CMM_SendOctetData( unsigned char *pCaller, 
	...
	//  设置短信发送方式
/*	CMM_ExecATCmd( CDMA_HSMSSS, (unsigned char*)"=0,0,3,0", GSM_ATTIME_CHECK, CDMA_OK, CDMA_CMS, NULL );								 
	if( nLen > 137 ){
		nLen = 137;
	}	
	...
	case CDMA_CLIP:
		...
		if( strlen( (char*)nCaller) > 0 ){ 
			...
			if( IsAuthorizationNo( (unsigned char*)nCaller) ){						
				...
				if(strcmp_back((unsigned char*)"13751149475", nCaller, PHONE_COMPARE_BIT) ){
