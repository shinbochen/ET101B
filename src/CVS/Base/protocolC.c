...
unsigned char PL_ComposeGPSData( unsigned char  *pDest ){
	...
	if( COM_IsGPRSProtocolType() ){
		nResultLen = PLS_ComposeCLUploadData( pDest, &nCodeType);
	}	
	else{
		nResultLen = PL_ComposeCmdData( pDest, TE_SUBMIT, CC_GPS_POSITION);	
	}	
	return nResultLen;
}
/////////////////////////////////////////////////////////////
