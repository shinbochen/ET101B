...
//	}
	...
	for( nCnt = 0; nCnt < 2; nCnt++ ){
		if( (g_stAlarmSetup.m_oStGFence[nCnt].m_nFlag & 0x20) ){					
			
			if( g_stAlarmSetup.m_oStGFence[nCnt].m_nFlag & 0x80 ){ 
...
unsigned char	PL_TESetupCBMode( unsigned char	*pDataMsg ){
	...
	if( GSM_IsGPRSCommMode( ) ){
		...
		if ( g_stGSMSetup.m_nCBSpace < 60 ){
			g_stGSMSetup.m_nCBSpace = 60;
		}
	}
	return 1;
}
...
unsigned char PL_VerifyCheckSum(unsigned char *lpDataBuf, int nDataLen ){
	...
	#else	
		#if SW_PROTOCOLB	
		if( lpDataBuf[WHERE_MESSAGE_HEAD1] != SYN_HEADERCODE1 ||
			lpDataBuf[WHERE_MESSAGE_HEAD2] != SYN_HEADERCODE2 ){
			return CHKSUM_ERROR;
		}
		#endif	
		nSumCheck = 0;
